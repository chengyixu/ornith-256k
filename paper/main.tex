...
\url{https://github.com/chengyixu/ornith-256k} (code) and the accompanying Hugging Face
repository. Hardware: MacBook Pro, Apple M4 Max (16-core), 64\,GB unified memory, macOS 27.0.}}
\date{August 26, 2026}
...
\begin{table}[h]
\centering
\caption{Identical-prompt streaming comparison (median of 3).}
\label{tab:h2h}
\begin{tabular}{lrrrr}
\toprule
Category & \multicolumn{2}{c}{TTFT med} & \multicolumn{2}{c}{Decode med (tok/s)} \\
 & Dense & MoE & Dense & MoE \\
\midrule
short-answer & 0.595\,s & 0.044\,s & 51.52 & \textbf{72.29} \\
code-gen     & 0.996\,s & 0.046\,s & 31.22 & \textbf{43.17} \\
summarize    & 1.537\,s & 0.033\,s & \textbf{53.68} & 66.23$^{*}$ \\
long-form    & 0.835\,s & 0.052\,s & 9.12 & \textbf{50.98} \\
reasoning    & 1.425\,s & 0.046\,s & 9.18 & \textbf{36.99} \\
\bottomrule
\end{tabular}

\smallskip
{\footnotesize $^{*}$Note: the MoE value already exceeds the dense value; bold marks the
winner per column. The summarize category differs between suites (32k cold prefill for
the MoE suite; short prompt in the head-to-head re-run), hence the asterisked pairing.}
\end{table}

The MoE wins four of five decode categories, with the largest margin on
open-ended generation (long-form, $5.6\times$): DFlash~2's speculative
acceptance collapses on prose, while the dense model's reasoning chains add
...
\begin{tabular}{lrrr}
\toprule
Task & Dense & MoE & Speedup \\
\midrule
read file        & 33.8\,s & 3.1\,s & $10.9\times$ \\
code write       & 45.1\,s & 4.5\,s & $10.0\times$ \\
debug (fixture)  & ---$^{\dagger}$ & 7.2\,s & --- \\
explain          & 268.6\,s & 21.2\,s & $12.7\times$ \\
refactor         & 139.0\,s & 11.2\,s & $12.4\times$ \\
multi-step shell & 75.4\,s & 5.5\,s & $13.7\times$ \\
\bottomrule
\end{tabular}

\smallskip
{\footnotesize $^{\dagger}$The debug fixture was absent during the first pass for both
models; the MoE was re-run with a planted off-by-one bug and fixed it in 7.2\,s. The
dense server became unavailable (weights removed amid disk pressure) before its re-run.}
\end{table}
...
\item Head-to-head pi suite: \texttt{bench/run\_pi\_bench.sh},
      \texttt{bench/bench\_pi.sh}, \texttt{bench/bench\_pi\_ornith.sh}
\item Raw records: \texttt{results/raw/*.jsonl} (streaming, cached-prefix,
      context proof, QA, agent turns, head-to-head)
...
\bibitem{localflash} chengyixu. \emph{Losing the Draft, Keeping the Speed: Block-Diffusion Speculative Decoding and Prefix-Cached Serving for Long-Context Agentic LLM Workloads on Apple Silicon}. Personal-systems research report, August 2026. \url{https://github.com/chengyixu/qwen38-dflash2-bench}.
